...
import { useCreateDrill, useDeleteDrill, useDrills } from "@poker-trainer/query";
  ...
  return (
    <div className="page">
      <header className="header">
        <h1>Poker Trainer</h1>
        <p className="muted">Boilerplate: drills CRUD via shared React Query + openapi-fetch client.</p>
        <nav>
        ...
        <ul className="list">
          {drills?.map((d) => (
            <li key={d.id} className="list-item">
              ...
              <button type="button" className="btn danger" onClick={() => deleteDrill.mutate(d.id)}>
                Delete
              </button>
            </li>
          ))}
      ...
      <section className="card">
        ...
        <form
          ...
          onSubmit={(e) => {
            ...
            if (!name.trim()) return;
            createDrill.mutate(
              { name: name.trim(), description: description || null, tags: null },
              {
                onSuccess: () => {
                  setName("");
                  setDescription("");
